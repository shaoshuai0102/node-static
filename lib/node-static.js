...
exports.Server = function (root, options) {
    ...
    if ('jsonp' in this.options) {
        this.jsonp = this.options.jsonp;
    }
...
exports.Server.prototype.respond = function (pathname, status, _headers, files, stat, req, res, finish) {
    ...
    var hook = query[this.jsonp];
    var jsonp = typeof hook == 'undefined' ? null : {start: hook + '(', end: ');'};
    ...
    if ((clientMTime  || clientETag) &&
        ...
        if (jsonp) {
            headers['content-length'] += (jsonp.start.length + jsonp.end.length);
            headers['content-type'] = mime.contentTypes['jsonp'];
        }
        ...
        if (this.cache && (key in exports.store) &&
            ...
            this.stream(pathname, files, new(buffer.Buffer)(headers['content-length']), jsonp, req, res, function (e, buffer) {
...
exports.Server.prototype.stream = function (pathname, files, buffer, jsonp, req, res, callback) {
    var offset = 0;
    if (jsonp) {
        res.write(jsonp.start);
        buffer.write(jsonp.start, 0, jsonp.start.length);
        offset += jsonp.start.length;
    }
    (function streamFile(files, offset) {
        ...
        if (file) {
            ...
            if (jsonp) {
                res.write(jsonp.end);
                buffer.write(jsonp.end, offset, jsonp.end.length);
                offset += jsonp.end.length;
            }
            res.end();
            callback(null, buffer, offset);
        }
    })(files.slice(0), offset);
};
